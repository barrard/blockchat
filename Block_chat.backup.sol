...
contract BlockChat{
    ...
    mapping (uint=>Room_object) id_to_room_mapping;
    ...
    function get_room_by_index (uint _index) constant public returns(uint, address, string, uint, bool, address[]) {
        Room_object memory _ro = id_to_room_mapping[_index];
        return (_ro._id, _ro._addr_of_creator, _ro._name, _ro._date, _ro._public, _ro._alowed_if_not_public);        
    }
    
    function get_room_by_name (string _name) constant public returns(uint, address, string, uint, bool, address[]) {
        uint _index = room_to_id_mapping[_name];
        Room_object memory _ro = id_to_room_mapping[_index];
        return (_ro._id, _ro._addr_of_creator, _ro._name, _ro._date, _ro._public, _ro._alowed_if_not_public);        
    }
    
    function delete_room(uint _index) public returns(bool){
        Room_object memory _ro = id_to_room_mapping[_index];
    ...
    function add_allowed_address_to_room(uint _room_index, address _addr) public returns(bool){
        Room_object storage _ro = id_to_room_mapping[_room_index];
        address _owner = _ro._addr_of_creator;
        if(_owner == msg.sender){
    ...
    function get_chat_room_chat_count(string _chat_room) constant public returns (uint){
        uint _room_id = room_to_id_mapping[_chat_room];
        Room_object memory _ro = id_to_room_mapping[_room_id];
        if(_ro._public == true){ return chats_per_room[_chat_room];
        }else{
